...
1. What is the output of this program? Inside addFive(): 15 In main(): 10
2. Why doesn't `value` change after calling `addFive()`?: It's calling value variable in main
3. Where does the variable `number` exist? addFive() function
...
value = 10
---------------------
5. What happens to `number` after the function returns? number gets added 
...
1. What is the output of this program? Inside investigate(): 15 5
2. Why does `score` change while `bonus` does not? because suspect is a non-const reference
3. Why is `clue` declared as a `const` reference?
4. What would happen if the function attempted to modify `clue`?The program will fail to compile
5. Draw the call stack while `investigate()` is executing.nvestigate() is on top of main(), with suspect referring to score and clue referring to bonus
...
1. What is the output of this program? 14
2. Why do we pass `&clue` instead of `clue`? Because the function expects a pointer (int*), so we pass the address of clue
3. What does `*evidence` do? It dereferences the pointer to access the value stored at that memory address.
4. Why is the `nullptr` check important? To avoid dereferencing a null pointer The if condition fails, the function does nothing, and the program continues safely.
5. What would happen if the function was called as `investigate(nullptr);`? The if condition fails, the function does nothing, and the program continues safely.
